...
def parse_geo_sra(pd_array_partition): 
    ...
    for samples in pd_array_partition.itertuples(index=False):
        ...
        # releasedate = samples.releasedate
        # if( not pd.isnull(samples.collection_date_sam)) :
        #     collection_date = samples.collection_date_sam[0]

        releasedate = None

        if( not pd.isnull(samples.releasedate)) :
            if(is_date(samples.releasedate)):
                 #releasedate = datetime.strptime(samples.releasedate, '%Y-%m-%d')
        ...
        if( not pd.isnull(samples.collection_date)) :
            ...
            if(is_date(samples.collection_date)):
                #collection_date = datetime.strptime(samples.collection_date, '%Y-%m-%d')
        ...
        na_term = ['NA','not applicable','Not applicable','missing','Missing','not collected','Not Collected','uncalculated']
        ...
        if('k' in attributes_df):
            ...
            for index, attribute in attributes_df.iterrows():
                ...
                if attribute['k'] in lat_lon_tag :
                    ...
                    if lat_lon not in na_term :

                        lat_lon_list = re.search(r"(.*[N|S])[,|\s](.*[W|E])",str(lat_lon))
                ...
                if attribute['k'] in lat_tag :
                    ...
                    if lat not in na_term :
                        lat_list = re.search(r"([N|S]?.*)|(.*[N|S]?)",str(lat))
                ...
                if attribute['k'] in lon_tag :
                    ...
                    if lon not in na_term :
                        lon_list = re.search(r"([W|E]?.*)|(.*[W|E]?)",str(lon))
        ...
        # append result
        array_result.append([bioproject,acc,organism,assay_type,instrument,librarylayout,libraryselection,librarysource,geo_loc_name_country_calc,geo_loc_name_country_continent_calc,mbytes,mbases,releasedate,collection_date,lat_lon_src,lat_lon_raw,latitude,longitude,latitude_precision,longitude_precision,has_latlon,rg_country_code,rg_city,rg_country])
